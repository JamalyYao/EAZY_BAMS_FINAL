...
import com.eazytec.common.util.office.ExportWord;
...
public class DwrOaNewsService {
	...
	/**
	 * 导出word JC 2014-02-08
	 * @param context
	 * @param request
	 * @param id
	 * @return
	 */
	public ResultBean exportAnnounce(ServletContext context, HttpServletRequest request, long id) {
		
		try {
			ExportWord.setRealPath(UtilTool.getRealPath(context));
			
			OaAnnouncement announcement = oaNewsService.getAnnouncementByPK(id);
			announcement.setOaAnnoText(ExportWord.changeToWordXml(announcement.getOaAnnoText()));
			
			Map<String,Object> map = new HashMap<String, Object>();
			map.put("an", announcement);
			
			ExportWord.exportAnnounce(map);
			
		}catch (IOException e) {
			e.printStackTrace();
			return new ResultBean(false,"向WORD中导入图片时出错，请检查上传的图片或者联系技术人员。");
		}catch (TemplateException e) {
			e.printStackTrace();
			return new ResultBean(false,"公告模板未定义，请联系技术人员。");
		}catch (Exception e) {
			e.printStackTrace();
			return new ResultBean(false,"导出文件出错，请联系技术人员。");
		}
