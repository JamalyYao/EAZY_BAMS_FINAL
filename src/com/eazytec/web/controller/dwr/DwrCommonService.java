...
public class DwrCommonService {
	...
	private ArrayList<String> getMethodIdsById(String code) throws Exception {
		...
		int slen = code.indexOf(ConstWords.SystemMethodInfoSign);
		if (slen != -1) {// 管理员账户菜单
			String tmp = code.substring(ConstWords.SystemMethodInfoSign.length());
			int strlen = tmp.length() / ConstWords.SysMethodInfoIdLenght;
			for (int i = 0; i < strlen; i++) {
				String s = tmp.substring(0, (i + 1) * ConstWords.SysMethodInfoIdLenght);
				mids.add(ConstWords.SystemMethodInfoSign + s);
			}
		} else {
	...
	public String getCodeByTable(ServletContext context, HttpServletRequest request, String upcode, String table, String colname, String upcol) {
		SessionUser user = (SessionUser) LoginContext.getSessionValueByLogin(request);
		int companyId = 0;
		if(table.equals("SYS_METHOD_INFO") || table.equals("SYS_LIBRARY_STANDARD")){
			//不做任何处理
		}else{
	...
	public List<SysLibraryInfo> getLibraryInfoByCode(ServletContext context, HttpServletRequest request, String code) {
		return sysProcessService.getSysLibraryInfoByCode(code, UtilTool.getCompanyId(request));
	}
	...
	public SysLibraryInfo getLibraryInfoByCodeAndCompanyId(ServletContext context, HttpServletRequest request, String code) {
		return sysProcessService.getSysLibraryInfoByCodeAndCompanyId(code, UtilTool.getCompanyId(request));
	}
	 ...
	 * @return
	 */
	public List<SysAttachmentInfo> saveAttachmentInfo(ServletContext context, HttpServletRequest request, String fileNames) {
