...
<html>
	<head>
		...
		<%
			...
			int row = 15;
			String title = usermsg.getCompanyName()+UtilTool.getSysParamByIndex(request,"erp.orderbackall.title");
		%>
		<title>退货单统计打印</title>
		<script type="text/javascript">
window.onload = function(){	
	var sArr = "<%=ids%>".split(",");
	dwrStorehouseService.getAstPurchaseRejectedByPks(sArr,setpagevalue);
}
...
function setpagevalue(data){
	if(data.success&&data.resultList.length>0){
		...
		for(i=0;i<data.resultList.length;i++){
			count += data.resultList[i].astRejectedCount;
		}
...
function creatDetail(data,count){
	...
	if(data.success&&data.resultList.length>0){
		...
		for ( var i = 0; i < len; i++) {
	        ...
	        if(dt.astOrderId!=null&&dt.astOrderId.length>0){
	        ...
	        td4.innerHTML =dt.astRejectedId;
			...
			td8.innerHTML= fmoney(dt.astRejectedCount,0) + dt.product.libraryInfoUnit.libraryInfoName;
			...
			td10.innerHTML =  dt.astRejectedDate;
