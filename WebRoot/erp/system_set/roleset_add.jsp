	 ...
	 %>
		...
		<DIV class="tagContentdiv" style="overflow: hidden;width: 99%">
			...
			<tr>
				...
				<input type="checkbox"  <%=chk1 %> value="<%=levlMethod.getPrimaryKey() %>" upvalue ="<%=levlMethod.getLevelUnit() %>" name="ck_<%=sysMethod.getMethodSign()%>" onclick="checkboxchange(this,'<%=levlMethod.getPrimaryKey() %>','<%=sysMethod.getMethodSign()  %>','<%=levlMethod.getLevelUnit()%>')" id="chk_<%=levlMethod.getPrimaryKey() %>"><label for="chk_<%=levlMethod.getPrimaryKey() %>"><%=levlMethod.getMethodInfoName() %></label>
				</td>
				 ...
				 <%for(int a=0;a<levelist2.size();a++){ 
					...
					if(levlMethod2.getLevelUnit().trim().equals(levlMethod.getPrimaryKey())){
				 	...
				 	<td style="width: 12%" nowrap="nowrap">
						<input type="checkbox" <%=chk2 %> value="<%=levlMethod2.getPrimaryKey() %>" upvalue ="<%=levlMethod2.getLevelUnit() %>" name="ck_<%=sysMethod.getMethodSign() %>" onclick="checkboxchange(this,'<%=levlMethod2.getPrimaryKey() %>','<%=sysMethod.getMethodSign()  %>','<%=levlMethod2.getLevelUnit()%>')" id="chk_<%=levlMethod2.getPrimaryKey() %>"><label for="chk_<%=levlMethod2.getPrimaryKey() %>"><%=levlMethod2.getMethodInfoName() %></label>
					</td>
					<td>
					...
					if(levlMethod3.getLevelUnit().trim().equals(levlMethod2.getPrimaryKey())){
				 	...
				 	<td nowrap="nowrap" style="padding-left: 5px;padding-right: 5px">
						<input type="checkbox" <%=chk3 %>  value="<%=levlMethod3.getPrimaryKey() %>" upvalue ="<%=levlMethod3.getLevelUnit() %>" name="ck_<%=sysMethod.getMethodSign() %>" onclick="checkboxchange(this,'<%=levlMethod3.getPrimaryKey() %>','<%=sysMethod.getMethodSign()  %>','<%=levlMethod3.getLevelUnit()%>')" id="chk_<%=levlMethod3.getPrimaryKey() %>"><label for="chk_<%=levlMethod3.getPrimaryKey() %>"><%=levlMethod3.getMethodInfoName() %></label>
					</td>
					<%
					td++;
					if (td > 0 && td % 5 == 0) {
...
window.onload=function(){
	useLoadingMassage(window.parent.parent);
