...
<script type="text/javascript">
	...
	function savecallback(data){
		if(<%=type%>!=null){
			...
			var frm = window.parent.parent.getMDIFrame(<%=request.getParameter("nowwindow")%>);
				...
				mad.value = tmps;
				confirmmsgAndTitle("添加联系人成功！是否想继续添加联系人？","reset();","继续添加","closeMDITab(window.parent.parent);","关闭页面");
			}else{
				alertmsg(data);
			}
		}else{
	...
	function closePage(){
			var frm = window.parent.parent.getMDIFrame(<%=request.getParameter("nowwindow")%>);
			if(typeof frm.netmail != 'undefined'){
				if(typeof frm.netmail.queryData == "function"){
					frm.netmail.queryData();	//调用原页面查询方法
				}
			}else{
				frm.queryData();
			}
