...
<%}else{%>
	function repSend(){
		...
		if(cf){
			...
			GetXmlHttp(delPaths,GetXmlValueByTagAll);
		}
	}
	...
	function deleteOkFile(a){
		...
		if(cf){
			...
			var tmp =dpath.split("|");
			trId=a;
			GetXmlHttp(tmp[1],GetXmlValueByTag);
		}
	}
	...
	function CreateXmlHttpRequest(){
	    ...
	    if (!xmlhttp) {
	        alert("XMLHttpRequest请求对象创建失败！");
	    }
	}
	
	function GetXmlHttp(filePaths,callbackMethod){
	    CreateXmlHttpRequest();
	    // 因为使用异步方式所以要在 XMLHttpRequest 对象的状态改变时做相应的处理
	    xmlhttp.onreadystatechange = callbackMethod;
	    xmlhttp.open("GET", "<%=request.getContextPath()%>/erp/fileProcess.jsp?filepath="+filePaths, true);
	    // 发送请求，因为是GET，所以send的内容为null
	    xmlhttp.send(null);
	}
	
	function GetXmlValueByTagOld(){
		if(xmlhttp.readyState == 4) {   
	      if(xmlhttp.status == 200) {   
	        var bl =trim(xmlhttp.responseText);
	        if(bl == "true"){
	        	//删除行
	        	deleteRow();
	        }else{
...
<tr>
<td>
<div style="background: #f6f6f6;border-bottom: 2px inset #cccccc;">
...
<a href="javascript:void(0);" class="files" title="添加附件"><input id="upfile1" name="upfile1" type="file" onchange="File.add(this)"></a>
</div>
</td>
<td align="center"  nowrap="nowrap" width="28" height="28">
...
%>
<td>&nbsp;&nbsp;<span style="font:12px '宋体';color:#666666;cursor: default;" id="msgBox">选择文件并上传[个数:<%=fileCountMsg %>&nbsp;&nbsp;&nbsp;大小:<%=fileSize %>M&nbsp;&nbsp;&nbsp;文件类型限制:<font title="<%=tmptitle %>" style="font:12px '宋体';color:#666666"><%=ext %></font>]</span></td> 
<%}else if(obj!=null){%>
<td align="center"  nowrap="nowrap" width="28" height="28">
<div class="addfilediv">
...
<img src='<%=request.getContextPath()%>/images/fileokico.png' border='0' width='16' height='16' style="vertical-align: middle;"/>&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick="showOkImg(this);"  onmouseout="hiddenOkImg();" value="<%=t%>" temp="<%=key %>"><%=key %></a>&nbsp;&nbsp;&nbsp;&nbsp;<a  href='javascript:void(0)' onclick='deleteOkFile(<%=i %>)' style='color:#336699'>删除</a>
