	...
	if(methodlist!=null&& methodlist.size()>0){
		...
		for(int i=0;i<methodlist.size();i++){
			...
			if(md.getMethodUri()!=null&&md.getMethodUri().trim().length()>0){
				action = "action=\"treeclick('"+md.getPrimaryKey()+"','"+request.getContextPath()+"/erp/"+md.getMethodUri()+"');\"";
			}
			//输出树节点
			sb.append("<tree id =\""+md.getPrimaryKey()+"\" text=\""+md.getMethodInfoName()+"\" value=\""+md.getPrimaryKey()+"\" "+tmp+" "+action+" color=\"#525151\" "+icon+"/>\n");
		}
		sb.append("</tree>");
