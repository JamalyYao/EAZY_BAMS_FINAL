...
window.onload = function() {
	...
	if('<%=type%>'=='1'){
		...
		if ( <%=pk %>!=null) {
			dwrPersonalOfficeSerivce.getTimedRecordByPk( <%=pk%>, setTimed);
		} else {
			$("#timed1").hide();
			selValue=0;
		}
...
function save() {
	...
	if (bl) {
		...
		if(selValue==0) {
			if(<%=pk%>!=null){
		    	dwrPersonalOfficeSerivce.saveTimedRecord(timedRecord,updateOaRefisterout);
	    	} else {
	    		dwrPersonalOfficeSerivce.saveTimedRecord(timedRecord,saveTimedRecord);
	    	}
		} else {
...
function closePage(){
	if ( <%=pk %>!=null) {
		...
		if(typeof frm.timedfrm != 'undefined'){
			frm.timedfrm.queryData();	//调用原页面查询方法
		}else{
			frm.queryData();
		}
...
function savedesktop(dialogId,frm,method){
	...
	if (bl) {
	    ...
	    var timed;
		if(selValue==0) {
	    	dwrPersonalOfficeSerivce.saveTimedRecord(timedRecord,saveDesktopCallback);
		} else {
			timed = document.getElementById("timedDate1");
		    var end = timed.value.split(" ");
		    var start = getFullCurrentDate().split(" ");
