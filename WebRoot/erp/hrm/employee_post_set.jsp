...
function queryData(){
	...
	var employeepks = tempUrlParameter.getTmpvalue(window.parent.parent);
	var pager = getPager();
...
function getPostUpid(){
    var box = SEL.getPostIds("radio","hrmEmployeePostIdtext","hrmEmployeePostId","processloadfrm");
    box.show();
}

function getPartPostUpid(){
    var box = SEL.getPostIds("check","hrmPartPosttext","hrmPartPost","processloadfrm");
    box.show();
}
</script>
</head>
...
sccList.add(new SysColumnControl("hrmDepartment.hrmDepName", "部门",
		1, 2, 2, 0));
sccList.add(new SysColumnControl("hrmEmployeePost.hrmPostName",
		"岗位", 1, 2, 2, 0));
sccList.add(new SysColumnControl("hrmPartPostName", "兼职岗位", 1, 2,
		2, 0));
...
<table align="center">
<tr>
<td><btn:btn onclick="save();" value="保 存 " imgsrc="../../images/png-1718.png" title="保存岗位设置" /></td>
