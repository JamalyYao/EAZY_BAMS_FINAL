...
function update(employeepk){
	Sys.MDIOpen("<%=contextPath%>/erp/hrm/employee_add.jsp?employeepk="+employeepk+"&nowwindow="+Sys.getWindow());
}
...
function exportok(){
	useLoadingMassage();
	var employee = getQueryParam();
	dwrHrmEmployeeService.exportEmployee(employee,exportCallback);
}

function exportCallback(data){
    if(data.success){
		confirmmsgAndTitle("导出人员信息成功！","downloadFile()","下载文件","","关闭");
	}else{
		alertmsg(data);
	}
}
...
function downloadFile(){
	location.href = "<%=contextPath %>/downloadfile/<%=ExportExcel.EMPLOYEE_FILE_NAME %>";
}
...
function print(){
	if(getAllRecordArray() != false){	
		...
		var empId = getAllRecordArray()[0];
		window.open('<%=contextPath%>/erp/show_report.jsp?raq=employee.raq&macrozgh=\\\'' + empId + '\\\'');
	}else{
		alertmsg("请选择要打印的人员...");
	}
}
