...
function setOptionResult(data){
	if(data != null && data.resultList.length>0){
		...
		for(var i = 0; i < data.resultList.length; i++){
			...
			var color = getColor();
			var tmp ="<label style='width:"+jc+"px;background-color:#ededed;border-top:1px solid #ededed;border-bottom:1px solid #808080;border-left:1px solid #ededed;border-right:1px solid #808080;'><span style='width:"+wi2+"px;background-color:" + color + ";color:#ffffff;'></span></label>";
			var ctmp="";
			if(chooseType == <%=EnumUtil.OA_VOTE_OPTIONS_TYPE.RADIO.value%>){
