...
//*************状态栏	***************//		
	...
	download : function(){
		...
		if(records.length <= 0){
			...
			if(records.length == 1 && records[0].data.leaf){
				window.open(encodeURI(encodeURI('download.action?path=' 
					+ tempPath 
	...
	var records = this.getSelectionModel().getSelections();
		...
		if(records.length ==1 && records[0].data.leaf){
			...
			var url = encodeURI(encodeURI("netdisk/netdisk_sharefolder.jsp?folderName="+folderName+"&folderPath="+shareFolderPath));
		...
		if (!grid.getStore().getAt(rowIndex).data.leaf) {// 如果文件夹才获访问
			...
			if(index == -1){
				...
				if(extensionName =='png' || extensionName=='jpg'|| extensionName=='gif'|| extensionName=='bmp') {
			    	 var url = encodeURI(encodeURI("showimageTemp.jsp?path="+path+'&isShare=' + this.isShare));
					 var win = new Ext.Window({
				  		  title : '图片预览',
				  		  autoHeight:true,
						  width:200,
