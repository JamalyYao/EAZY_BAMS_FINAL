	...
	function getdivContentByType(type){
		...
		if(type=='online'){
			...
			fd.style.display="";
			GetXmlOnlineCount(Sys.getProjectPath()+"/erp/desktop/getOnlineCount.jsp",content);
		}else if(type == 'sms'){
			sh.style.display="";
			fd.style.display="";
	...
	function GetOnlineCount(xh,obj){
		if(xh.readyState == 4) {
			if(xh.status == 200) {
	        	var str =trim(xh.responseText);
	        	obj.innerHTML = "<label style='float:left;margin-left: 5px'>当前系统在线人员</label><label style='float:right;margin-right: 5px'>["+str+"]</label>";
	        }
	    }
	}
	...
	function getApproveCount(){
		dwrPersonalProcessSerivce.listTaskTodo(null, setApproveResult);
	}
