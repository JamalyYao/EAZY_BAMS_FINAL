...
function alertmsg(data, successevent,errorevent, par) {
	...
	msgObj.onclick = function() {
		...
		if (successevent != null && successevent != "undefined" && successevent != undefined && successevent.length>0) {
			if(data.success||typeof(data)=="string"){
				eval(successevent);
			}
		}
		if (errorevent != null && errorevent != "undefined" && errorevent != undefined && errorevent.length>0) {
			if(!data.success){
				eval(errorevent);
			}
		}
...
function confirmmsg(msg,okevent,cancelevent,par) {
	if (par == null || par == "undefined" || par == undefined) {
		par = window.parent.parent.parent.parent;	//将页面级别升至顶
	}
	...
	/**
	msgObj.attachEvent('onkeyup', function(e){
		var e=e||event;
		...
		if (key==27){//esc
			...
			if (cancelevent != null && cancelevent != "undefined" && cancelevent != undefined && cancelevent.length>0) {
				eval(cancelevent);
			}
	    }
	});
	**/
	...
	oklink.onclick = function() {
		...
		if (okevent != null && okevent != "undefined" && okevent != undefined && okevent.length>0) {
			eval(okevent);
		}
	}
	
	/**
	oklink.attachEvent('onkeyup', function(e){
		var e=e||event;
		var key = e.keyCode||e.which;
		if (key==39){//→
			cancellink.focus();
	...
	cancellink.attachEvent('onkeyup', function(e){
		var e=e||event;
		var key = e.keyCode||e.which;
		if (key==37){//←
			oklink.focus();
