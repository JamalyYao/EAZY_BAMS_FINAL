...
function setselectStyle() {
	var menubl = typeof(Menu)=="function"?true:false;
	if(menubl){
		//右键菜单
		Menu.Config.FirstMenu.Create();
	    document.onmousedown=function(){
	    	currentInputObject.setobject(null);
	...
	for(var i=0;i<al;i++){
		...
		if(menubl&&area.getAttribute("readonly")){
			area.title += " 点击右键清除";
			area.oncontextmenu=function(e){
				currentInputObject.setobject(this);
        		e=e||event;
	...
	for (var i = 0; i <tl ; i++) {
		...
		if ( ta== "text" || ta == "password") {
			...
			if (cn == null || cn == "undefined" || cn == undefined || cn.length == 0) {
				...
				txt.title += " 点击右键清除";
				if(menubl&&txt.getAttribute("readonly")){
