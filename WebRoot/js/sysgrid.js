...
function initGridData(resultList,pager){
	...
	try{
		...
		if(sysGridConfing.showview==sysGridShowView.showAll || sysGridConfing.showview == sysGridShowView.showTable){
			var rlen = resultList.length;
			var roundlen = pager.pageSize-rlen;
			for(var i=0;i<rlen;i++){
				...
				otrCount.style.cssText = "BACKGROUND-IMAGE: url("+sysGridConfing.conpath+"/images/grid_images/fhbg2.gif);";
				...
				//otr.setAttribute("onmouseover",function(){SetTRcolorOver(this,'tableContentOn001')});
				otr.onmouseover=function(){
					SetTRcolorOver(this,'tableContentOn001');
				};
				
				//otr.setAttribute("onmouseout",function(){SetTRcolorOut(this)});
				otr.onmouseout=function(){
					SetTRcolorOut(this);
				};
				
				//otr.setAttribute("onclick",function(){clickto()});
				...
				otr.value = vl;
				//otr.setAttribute("ondblclick",function(){try{eval(sysGridConfing.dblfunction+"(this)")}catch(err){}});
				...
				for(var j=0;j<sysGridConfing.collist.length;j++){
					...
					var cs = 'line-height: 15px;';
					if(parseInt(pars[2])>0){
				...
				if(sysGridConfing.isshowProcess){
					...
					processtr.style.cssText = "BACKGROUND-IMAGE: url("+sysGridConfing.conpath+"/images/grid_images/fhbg2.gif);";
			...
			if(sysGridConfing.isshowBlank){
				for(var i=0;i<roundlen;i++){
					...
					for(var j=0;j<sysGridConfing.collist.length;j++){
						var td = document.createElement("td");
						td.style.cssText ="line-height: 15px;";
						td.innerHTML = "&nbsp;";
						otr.appendChild(td);
					}
					if(sysGridConfing.isshowProcess){
		...
		if(sysGridConfing.showview==sysGridShowView.showAll || sysGridConfing.showview ==sysGridShowView.showImage){
			...
			for(var i=0;i<resultList.length;i++){
				...
				var vl = eval("obj."+sysGridConfing.dblbundle);
				td.value = vl;
				//td.setAttribute("ondblclick",function(){try{eval(sysGridConfing.dblfunction+"(this)")}catch(err){}});
				...
				//td.setAttribute("onclick",function(){clickimg();});
				
				td.onclick=function(){
					clickimg();
				};
				
				//td.setAttribute("onmouseover",function(){imgmouseover(this);imgshowdetail(this);});
				
				td.onmouseover=function(){
					imgmouseover(this);imgshowdetail(this);
				};
				
				//td.setAttribute("onmouseout",function(){imgmouseout(this);imghiddendetail(this);});
...
function imgshowdetail(img){
	...
	if(imgdiv.style.visibility == "visible") return;
...
function selectcpage(obj){
	...
	var pdivhei = pdiv.clientHeight;
	var pdivwidth = pdiv.clientWidth;
    ...
    pdiv.style.top = ttop - pdivhei - 7;
    pdiv.style.left = tleft - pdivwidth/2+18;
...
function pagetosend(){
	...
	document.getElementById("pagesenddiv").style.visibility = "hidden";
}
